...
<body style="margin:0px; padding:0px;">
	...
	<div style="float:right; padding-left:8.4px; background-color:teal; margin:0px; padding-bottom:20px">
	 <a href="http://accsectiondemo.site11.com/test2.html" style=" text-align:center; color:yellow; text-decoration:none; padding: 30px; font-size:24px; font-weight:bold"> <img src="homeicon.png" style="display:inline; height: 40px; width:35px; margin-right:5px;margin-top:13px;">  <?php echo "HOME" ?>  </a></li>  </a>
	 </div>
	...
	<div style="position:absolute; margin-top:12%; background-color:transparent; width:45%; height:330px; color:white; margin-left: 26%;">
		...
		<form method= "post" action="http://localhost/assignment/login2.php" >
		...
		<a href="http://accsectiondemo.site11.com/SignupPage.php" style="  margin-left: 38%;color:white; font-size:24px; font-weight:bold"> Sign up here </a>
	...
	if(!empty ($username) && !empty($password)) {
		...
		if($query_run = mysql_query($query)) {
			...
			if($query_num_rows ==0) {
				...
				$user_firstname = mysql_result($query_run,0,'First_Name');
				$user_lastname = mysql_result($query_run,0,'Last_Name');
				...
				$roll = mysql_result($query_run,0,'Roll_Number');
				$_SESSION['roll2'] = $roll;
				$dept = mysql_result($query_run,0,'Department');
				$_SESSION['dept']=$dept;
				$year = mysql_result($query_run,0,'year');
				$_SESSION['yr']=$year;
				$semester = mysql_result($query_run,0,'semester');
				...
				else 	header('Location: fullprofile_test.php');
			}
		}
	}
	else {
